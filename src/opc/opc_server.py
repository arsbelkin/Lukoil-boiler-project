...
class OPCBoilerServer:
    ...
    ):  # функция для определения сервера
        self.server = Server()
    ...
    def get_value(
        self, name
    ):  # функция для получения значения определенного датчика из бойлера
        return self.nodes[name].read_value()

    def set_value(
        self, name, value
    ):  # функция для изменения значения, например поставить кран с 0.5 на 0.7
